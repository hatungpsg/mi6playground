...
def store_and_refresh_token(conv_id=None):
    ...
    try:
        while True:
            ...
            tcd_reference_file_path = 'mQSPI_TCD_Test.txt'
            tcd_reference_file = open(tcd_reference_file_path, 'r')

            # Step 2: Read the contents of the file
            tcd_reference_file_contents = tcd_reference_file.read()
            nontcd_folder_path = 'tcd'
            files_contents = read_all_py_files_in_folder(nontcd_folder_path)
            # Print the contents of each file

            for file_name, content in files_contents.items():
                print(f"Below is the {file_name}")
                payload = {
                    "query": f"Below is the {file_name}. help me using '##' as label to insert comments into \n\n {content} without skip anyline of the code after study mQSPI_TCD_Test.txt:\n\n{tcd_reference_file_contents}",
                ...
                if apiResponse.status_code == 200:
                    ...
                    if response_data:
                        ...
                        tcd_file_name = 'tcd_'+file_name
                        # Example usage
                        file_path = 'tcd/'+tcd_file_name
