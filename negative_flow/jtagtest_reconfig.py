...
def main():
    ...
    try:
        ...
        if not emulator:
            ...
            ## Load signed encrypted bitstream 2 via JTAG if recovery flow
            #/ Fail to load with 0xf0090054
            if recovery == 1:
                loop_pass = 0
                for count in range(reconfig_count):
                    print("TEST :: Loop %d for recovery"%count)
                    local_pass = 1

                    try:
                        fwval_lib.print_stdout()
                        print("TEST :: Expected failed in configuration using AES key mismatched bitstream")
                        local_checks=[]

                        local_checks.extend(test.complete_jtag_config(file_path=rbf_signed2, success=0, failed_state=0xf0090054, before_cmf_state=1, skip=1, skip_ver=1,
                            exp_err="JTAG programming time exceeds the maximum", failed_cmf_state=1,test_mode=test_mode))
                        ...
                        fwval_lib.print_err("ERROR :: Unexpected result for configuration using AES key mismatched bitstream")
                    ...
                    try:
                        ...
                        local_checks.extend(test.complete_jtag_config(file_path=rbf_signed, before_cmf_state=1, skip=1, skip_ver=1, success=1,test_mode=test_mode))
                        local_checks.append(test.verify_design_andor(rbf_signed))
                        ...
                        fwval_lib.print_err("ERROR :: Unexpected result for configuration using AES key matched bitstream")
                ...
                print("TEST :: %d/%d loop is passing for recovery test" %(loop_pass, reconfig_count))

            if reprogram_aeskey == 1 and key_storage == "BBRAM" :
                print ("TEST :: Reconfigure using signed encrypted bitstream with qek file - %s" %qek_file)
                # Program AES Key 2 into BBRAM using SDM Command thru JTAG/Programmer
                fwval_lib.print_stdout()
                if test_flow == "prov":
                    print("TEST :: To bring up provision firmware")
                    ...
                    try:
                        ...
                        ## Load provision firmware if BBRAM and recovery flow
                        #/ Load successfully
                        error = testOBJ.security_provisioning(dutHANDLE = test, sha_type='sha384', state=0x00000000, skip_ver=1,test_mode=test_mode)
                        checks.append(error)

                        # Update expectation for provision_cmf back to 0 to prepare for next config
                        test.update_exp(nconfig=1, nstatus=1, provision_cmf=0)

                    except:
                        #log the traceback into stderr
                        logging.exception('')

                        fwval_lib.print_err("ERROR :: Failed to bring up provision fw!")

                ## Clear AES key if BBRAM and recovery flow
                #/ Pass
                test.jtag_volatile_aes_erase()
                ...
                try:
                    ...
                    # Program AES Key
                    local_checks.append(test.program_aeskey(qek=qek_file, passphrase_file=passphrase_file, option=qek_program, pem_file=pem_file_aes, qky_file=qky_file_aes,
                        key_storage=key_storage,debug_programmerhelper=debug_programmerhelper, success=True))
                ...
                if recovery == 1:
                    try:
                        ...
                        local_checks.extend(test.complete_jtag_config(file_path=rbf_signed2, success=0, skip_ver=1, failed_state=0xf0090054, before_cmf_state=1, skip=1,
                            exp_err="JTAG programming time exceeds the maximum", failed_cmf_state=1,test_mode=test_mode))
                ...
                ## Load signed encrypted bitstream 1 via JTAG if recovery flow
                #/ Bitstream loaded successfully
                try:
                    print("TEST :: Expected passed in configuration using AES key matched bitstream")
                    local_checks=[]
                    ...
                    local_checks.extend(test.complete_jtag_config(file_path=rbf_signed, before_cmf_state=1, skip=1, skip_ver=1, success=1,test_mode=test_mode))
                    local_checks.append(test.verify_design_andor(rbf_signed2))
